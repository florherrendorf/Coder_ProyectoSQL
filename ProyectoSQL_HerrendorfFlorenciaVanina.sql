...
INSERT INTO repartos (id_reparto, id_patente, id_repartidor)
...
,		d.barrio
,		l.localidad
,		timestampdiff(year, r.fecha_ingreso, curdate()) antiguedad
,		(	SELECT	count(ru.id_reparto) 
			FROM	rutas ru
            ,		repartos re
            WHERE	re.id_repartidor = r.id_repartidor
            AND		re.id_reparto = ru.id_reparto 
            ) clientes_asignados
FROM	repartidores r
,		domicilios d
,		localidades l
WHERE	d.id_domicilio = r.id_domicilio
AND		d.id_localidad = l.id_localidad
...
CREATE OR REPLACE VIEW vw_clientes_por_localidad AS 
SELECT 	l.*
...
,		domicilios d
,		clientes_domicilios cd
WHERE	l.id_localidad = d.id_localidad
AND		d.id_domicilio = cd.id_domicilio
AND		c.id_cliente = cd.id_cliente
AND		cd.id_tipo_domicilio = 'A'
...
-- Bidones de 20 litros venidos por mes, por cliente y por medio de pago.

CREATE OR REPLACE VIEW vw_b20_vendidos_cliente_mes AS 
SELECT 	last_day(t.fecha) periodo
,		o.operacion
,		mp.medio_pago
,		c.id_cliente
,		c.apellido_nombre
,		t.id_producto
,		sum(t.cantidad) cantidad_vendida
FROM	transacciones t
,		operaciones o
,		medios_pago mp
,		clientes c
WHERE	t.id_operacion = o.id_operacion
AND		t.id_medio_pago = mp.id_medio_pago
AND		t.id_cliente = c.id_cliente
AND		o.id_operacion = 'V'
AND		t.id_producto = 'B20'
GROUP BY	last_day(t.fecha)
,			o.operacion
,			mp.medio_pago
,			c.id_cliente
ORDER BY	periodo ASC, cantidad_vendida DESC
;
...
DROP PROCEDURE IF EXISTS sp_asignacion_repartidor_vehiculo;

DELIMITER $$
CREATE PROCEDURE sp_asignacion_repartidor_vehiculo (IN p_id_reparto VARCHAR(3), IN p_id_repartidor INT, IN p_id_patente VARCHAR(7))
BEGIN
	/*
...
/* -- Invocación procedimiento
CALL sp_asignacion_repartidor_vehiculo(  'R01'  , 1  , 'AA351EF'  );
CALL sp_asignacion_repartidor_vehiculo(  'R02'  , 2  , 'AA351EF'  ); 	-- hace rollback por duplicado vechiculo
CALL sp_asignacion_repartidor_vehiculo(  'R02'  , 13  , 'AC688WR'  );	-- hace rollback por repartidor inexistente
CALL sp_asignacion_repartidor_vehiculo(  'R02'  , 2  , 'AC688WR'  ); 	-- commit
CALL sp_asignacion_repartidor_vehiculo(  'R02'  , 3  , 'OTR896'  ); 	-- commit
CALL sp_asignacion_repartidor_vehiculo(  'R11'  , 2  , 'AC688WR'  ); 	-- commit

SELECT * FROM reparto_agua.repartos;
...
DROP USER IF EXISTS 'flor'@'localhost' ;
CREATE USER 'flor'@'localhost' IDENTIFIED BY 'unpass' ;

-- Dar permisos de lectura sobre las vistas
GRANT SELECT ON reparto_agua.vw_b20_vendidos_cliente_mes TO 'flor'@'localhost';
GRANT SELECT ON reparto_agua.vw_clientes_activos TO 'flor'@'localhost';
GRANT SELECT ON reparto_agua.vw_clientes_por_localidad TO 'flor'@'localhost';
GRANT SELECT ON reparto_agua.vw_empleados_activos TO 'flor'@'localhost';
GRANT SELECT ON reparto_agua.vw_total_clientes_por_reparto TO 'flor'@'localhost';

/* Control de permisos otorgados
SHOW GRANTS FOR 'flor'@'localhost';
*/
